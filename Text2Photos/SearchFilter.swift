...
public let searchExamples: [[String: String]] = [
    ...
    [
        "question": "photos taken near San Francisco",
        "answer": "SELECT a.ZUUID FROM ZASSET a WHERE a.ZTRASHEDSTATE = 0 AND a.ZKIND = 0 AND a.ZLATITUDE BETWEEN 37 AND 38 AND a.ZLONGITUDE BETWEEN -123 AND -122 ORDER BY a.ZDATECREATED DESC LIMIT 50"
    ],
    [
    ...
    [
        "question": "photos of Tina and Gene",
        "answer": "SELECT a.ZUUID FROM ZASSET a WHERE a.ZTRASHEDSTATE = 0 AND a.ZKIND = 0 AND EXISTS (SELECT 1 FROM ZDETECTEDFACE df JOIN ZPERSON p ON p.Z_PK = df.ZPERSONFORFACE WHERE df.ZASSETFORFACE = a.Z_PK AND p.ZDISPLAYNAME LIKE '%Tina%') AND EXISTS (SELECT 1 FROM ZDETECTEDFACE df JOIN ZPERSON p ON p.Z_PK = df.ZPERSONFORFACE WHERE df.ZASSETFORFACE = a.Z_PK AND p.ZDISPLAYNAME LIKE '%Gene%') ORDER BY a.ZDATECREATED DESC LIMIT 50"
    ],
    [
        "question": "photos of my favorite people",
        "answer": "SELECT a.ZUUID FROM ZASSET a INNER JOIN ZDETECTEDFACE df ON df.ZASSETFORFACE = a.Z_PK INNER JOIN ZPERSON p ON p.Z_PK = df.ZPERSONFORFACE WHERE a.ZTRASHEDSTATE = 0 AND a.ZKIND = 0 AND p.ZDISPLAYNAME IS NOT NULL AND p.ZDISPLAYNAME != '' GROUP BY a.ZUUID ORDER BY COUNT(DISTINCT p.Z_PK) DESC LIMIT 50"
    ]
]

public func generateDynamicSearchFilter(from searchText: String, client: GenerationClientProtocol) async throws -> String {
...
"""
    ...
    do {
        let rawSQLString = try await client.generate(query: query, systemInstructions: systemInstructions)
